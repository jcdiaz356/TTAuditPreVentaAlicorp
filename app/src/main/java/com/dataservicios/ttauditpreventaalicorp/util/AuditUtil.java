package com.dataservicios.ttauditpreventaalicorp.util;
...
//        }
    ...
    public static ArrayList<Product> getListProduct(int company_id, int competity){
        int success ;
        ...
        try {
            ...
            params.put("competencia", String.valueOf(competity));
            params.put("company_id", String.valueOf(company_id));

            JSONParserX jsonParser = new JSONParserX();
            // getting product details by making HTTP request
            JSONObject json = jsonParser.makeHttpRequest(GlobalConstant.dominio + "/ajaxGetProductsForCity" ,"POST", params);
            // check your log for json response
            Log.d("Login attempt", json.toString());
            // json success, tag que retorna el json
            if (json == null) {
                ...
                if (success == 1) {
                    ...
                    if(ObjJson.length() > 0) {
                        for (int i = 0; i < ObjJson.length(); i++) {
                            ...
                            product.setProduct_id(Integer.valueOf(obj.getString("id")));
                            product.setCompany_id(Integer.valueOf(obj.getString("company_id")));
                            product.setFullname(String.valueOf(obj.getString("fullname")));
                            product.setImagen(String.valueOf(obj.getString("imagen")));
                            product.setRegion(String.valueOf(obj.getString("region")));
                            product.setCantidad("");
                            product.setCompetity(Integer.valueOf(obj.getString("competencia")));
                            list.add(i,product);
                        }
                    }
                    Log.d(LOG_TAG, "Ingresado correctamente");
